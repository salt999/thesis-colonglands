...
healthy_dict = {'HEALTHY_FEATURES':[], 'lang':[], 'job':[]}
healthylist = []
healthylist_WHO = []
hyperlist = []
hyperlist_WHO = []
        ...
        if row[LANGUAGE] == 'de':
            ...
            for id in range(len(HEALTHY_FEATURES)):
                if len(row[HEALTHY_FEATURES[id]]) > 1:
                    #healthy_dict['HEALTHY_FEATURES'].append(row[HEALTHY_FEATURES[id]])
...
for enum, result in enumerate(ALL):
    ...
    with open(folder + resultfiles[enum] + extendfolder + '.csv', mode='w', encoding='utf8', newline ='') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=[resultfiles[enum],"", "OBJEKT" ,"BAUM STRUKTUR", "","SYNONYMS-woxikon","", "SYNONYMS-thesaurus", "HIERARCHY"])
        writer.writeheader()
        #writer.writerow(result)
        for res in result:
            writer.writerow({resultfiles[enum]: res, "OBJEKT": object_dict[res], "SYNONYMS-woxikon": object_syno2[res], "SYNONYMS-thesaurus": object_syno1[res]})  #"SYNONYMS-thesaurus": object_syno1[res], "SYNONYMS-woxikon": object_syno2[res]
